...
class Hunyuan3DClient:
    ...
    def generate(self, image_path: Path, output_path: Path) -> dict[str, Any]:
        ...
        if self.settings.hunyuan_mode != "local_api":
            raise RuntimeError(f"지원하지 않는 HUNYUAN_MODE: {self.settings.hunyuan_mode}")
        if not image_path.exists():
            raise FileNotFoundError(f"선택된 2D 이미지가 없음: {image_path}")

        image_b64 = base64.b64encode(image_path.read_bytes()).decode("ascii")
        payload = {"image": image_b64, "texture": self.settings.hunyuan_texture, "remove_background": True, "type": "glb"}
        ...
        with httpx.Client(timeout=timeout) as client:
            ...
            if "model/gltf-binary" in content_type or "application/octet-stream" in content_type:
                ...
                return {"mode": "binary", "source": endpoint}
            data = response.json()
        ...
        for key in ("file_path", "path", "output_path"):
            ...
            if value:
                ...
                if candidate != storage_root and storage_root not in candidate.parents:
                    raise RuntimeError("Hunyuan3D가 허용되지 않은 서버 파일 경로를 반환함")
                if candidate.is_file():
                    shutil.copy2(candidate, output_path)
                    return
        raise RuntimeError("Hunyuan3D API 응답에서 GLB 파일을 찾을 수 없음")

...
class LocalGemmaClient:
    ...
    def _fallback(self, prompt: str, category: str, image_paths: list[str]) -> dict[str, Any]:
        ...
        directions = [
            ("Compact", "compact footprint with tightly integrated modules"),
            ("Open Frame", "open structural frame exposing the operating mechanism"),
            ("Safety Cover", "transparent safety enclosure with front access"),
            ("Premium Cell", "fully integrated industrial cell with control panel and status lighting"),
        ]
        variants = []
        ...
        return {
            ...
            "uncertainties": ["정확한 치수와 표준 부품 규격은 별도 확인이 필요함"] if not image_paths else [],
